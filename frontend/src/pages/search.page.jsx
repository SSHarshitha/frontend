    ...
    const searchBlogs = ({ page = 1, create_new_arr = false }) => {
        ...
        axios.post(import.meta.env.VITE_SERVER_DOMAIN + "/search-blogs", { query, page })
            .then(async ({ data }) => {
                ...
                // Filter out duplicate blog entries by checking blog IDs
                let uniqueBlogs = formatedData.results.filter((value, index, self) =>
                    index === self.findIndex((t) => (
                        t.blog_id === value.blog_id
                    ))
                );

                setBlog({
                    ...formatedData,
                    results: uniqueBlogs // Set unique blog results
                });
            })
            .catch((err) => {
                console.log(err);
